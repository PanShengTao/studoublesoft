...
import doublesoft.android.stu.wheelview.OnWheelChangedListener;
import doublesoft.android.stu.wheelview.OnWheelScrollListener;
import doublesoft.android.stu.wheelview.WheelView;
import doublesoft.android.stu.wheelview.adapter.AbstractWheelTextAdapter1;
...
public class ChangeAddressPopwindow extends PopupWindow implements View.OnClickListener {
    ...
    private String strProvince = "贵州省";
    private String strCity = "贵阳市";
    private String strArea = "南明区";
    private OnAddressCListener onAddressCListener;

    private int maxsize = 14;
    private int minsize = 12;

    public ChangeAddressPopwindow(final Context context, String initProvince, String initCity, String initArea) {
        super(context);
        this.context = context;
        this.setAddress(initProvince, initCity, initArea);

        View view = View.inflate(context, doublesoft.android.stu.R.layout.edit_changeaddress_pop_layout, null);

        wvProvince = (WheelView) view.findViewById(doublesoft.android.stu.R.id.wv_address_province);
        wvCitys = (WheelView) view.findViewById(doublesoft.android.stu.R.id.wv_address_city);
        wvArea = (WheelView) view.findViewById(doublesoft.android.stu.R.id.wv_address_area);
        lyChangeAddress = view.findViewById(doublesoft.android.stu.R.id.ly_myinfo_changeaddress);
        lyChangeAddressChild = view.findViewById(doublesoft.android.stu.R.id.ly_myinfo_changeaddress_child);
        btnSure = (TextView) view.findViewById(doublesoft.android.stu.R.id.btn_myinfo_sure);
        btnCancel = (TextView) view.findViewById(doublesoft.android.stu.R.id.btn_myinfo_cancel);
    ...
    private class AddressTextAdapter extends AbstractWheelTextAdapter1 {
        ...
        protected AddressTextAdapter(Context context, ArrayList<String> list, int currentItem, int maxsize,
                                     int minsize) {
            super(context, doublesoft.android.stu.R.layout.item_birth_year, NO_RESOURCE, currentItem, maxsize, minsize);
            this.list = list;
            setItemTextResource(doublesoft.android.stu.R.id.tempValue);
        }
    ...
    private void initJsonData() {
        try {
            ...
            //通过管理器打开文件并读取
            BufferedReader bf = new BufferedReader(new InputStreamReader(assetManager.open("city.html")));
            String line;
    ...
    public void initProvinces() {
        ...
        arrProvinces.addAll(Arrays.asList(mProvinceDatas).subList(0, length));
    }
    ...
    public void initCitys(String[] citys) {
        if (citys != null) {
            ...
            arrCitys.addAll(Arrays.asList(citys).subList(0, length));
        } else {
            String[] city = mCitisDatasMap.get("贵州省");
            arrCitys.clear();
            int length = city.length;
            arrCitys.addAll(Arrays.asList(city).subList(0, length));
        }
    ...
    public void initAreas(String[] areas) {
        if (areas != null) {
            ...
            arrAreas.addAll(Arrays.asList(areas).subList(0, length));
        } else {
            String[] area = mAreaDatasMap.get("贵阳市");
            arrAreas.clear();
            int length = area.length;
            arrAreas.addAll(Arrays.asList(area).subList(0, length));
        }
