...
public class ProgressView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //进度条宽度 高度
        int height = myFunc.dip2px(mContext, pvHeight);
        ...
        valuePaint.setTextSize(myFunc.dip2px(mContext, pvFontSize));

        float valueRectWidth = valuePaint.measureText(pvValue + "%");
        canvas.drawText(pvValue + "%", (toX + height / 2) > valueRectWidth ? (toX + height / 2) - valueRectWidth : 0, height + myFunc.dip2px(mContext, pvVSpace + 10), valuePaint);
    }
